...
export default function GrievancePage() {
  ...
  function submit() {
    setRef("GRV-MOCK-" + Date.now().toString().slice(-6));
    setStep(4);
  }
      ...
      <main className="max-w-lg mx-auto px-4 pt-6">
        ...
        {step === 1 && (
          <div className="space-y-4">
            ...
            <textarea
              ...
              rows={4}
              placeholder="e.g. My pension payment has not arrived"
              className="w-full px-4 py-3 rounded-xl border border-slate-300 text-base"
            />
            <button
              type="button"
              disabled={what.trim().length < 10}
              onClick={() => setStep(2)}
              className="w-full bg-indigo-700 text-white font-semibold py-3 rounded-xl disabled:opacity-50"
            >
              Continue
            </button>
          </div>
        ...
        {step === 3 && (
          <div className="space-y-4">
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. Payment released and confirmation"
              className="w-full px-4 py-3 rounded-xl border border-slate-300"
            />
            <div className="bg-white rounded-xl border border-slate-200 p-3 text-sm text-slate-600">
              <p className="font-medium text-slate-800 mb-1">Summary (auto)</p>
              <p>{what}</p>
        ...
        {step === 4 && (
          <div className="text-center space-y-4 py-6">
            ...
            <p className="text-sm text-slate-600">Status: Submitted · Next: await department response (simulated)</p>
            <Link href="/" className="inline-block bg-indigo-700 text-white font-semibold px-6 py-3 rounded-xl">
              Back home
            </Link>
          </div>
        )}
